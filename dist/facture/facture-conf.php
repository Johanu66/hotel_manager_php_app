...
if (isset($_POST['id_location_conf'])){
  $query = "SELECT id_facture_conf FROM facture_conf
  WHERE id_location_conf_fk_facture_conf = :id_location_conf
  ";        
  ...
  foreach($result as $row) {
    $id_facture_conf = $row['id_facture_conf'];
  }
  ...
  $id_facture_conf = $_POST['id_facture_conf'];
        ...
        $query = "SELECT * FROM location_conf, facture_conf, salle_conf, client, personne
        ...
        AND facture_conf.id_location_conf_fk_facture_conf = location_conf.id_location_conf
        AND id_facture_conf = :id_facture_conf
        ";        
        $statement = $connect->prepare($query);
        $statement->execute(
          array(
            ':id_facture_conf'  =>  $id_facture_conf
          )
        );
        $result = $statement->fetchAll();

        foreach($result as $row) {
          $infos_client = $row['nom_personne'] . ' ' . $row['prenom_personne'] . '<br>' .
            $row['tel_personne'];

          $num_facture = $row['num_facture_conf'];
          $date = $row['date_facture_conf'];
          ...
          $montant_ttc_en_lettre = $row['montant_ttc_en_lettre_facture_conf'];
          $tva = (double) $row['valeur_tva_facture_conf'] * (double) $montant_ht / 100;

          $produit = formatProduit($str_produit, $row['nom_salle_conf'], $row['date_debut_location_conf'], $row['date_fin_location_conf']);
        }
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-hover table-md">
                    ...
                    <tr>
                      <td>1</td>
                      <td>'. $produit .'</td>
                      <td class="text-center">' . $montant_ht .'</td>
