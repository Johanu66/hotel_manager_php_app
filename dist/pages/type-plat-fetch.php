...
$colonne = array("id_type_plat", "nom_type_plat", "desc_type_plat","statut_type_plat");

$query = '';

$output = array();

$query .= "SELECT * FROM type_plat "; // changer

if(isset($_POST["search"]["value"]))
{	// changer les colonnes à rechercher
	$query .= 'WHERE nom_type_plat LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR statut_type_plat LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR desc_type_plat LIKE "%'.$_POST["search"]["value"].'%" ';
...
{
	$query .= 'ORDER BY id_type_plat DESC ';
}
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['id_type_plat'];
	$sub_array[] = $row['nom_type_plat'];
	$sub_array[] = $row['desc_type_plat'];
	$sub_array[] = $status;
	...
	<div class="btn-group">
      ...
      <div class="dropdown-menu">
        <a class="dropdown-item view" id="'.$row["id_type_plat"].'" href="#">'. $consulter . '</a>
        <a class="dropdown-item update" id="'.$row["id_type_plat"].'" href="#">'. $modifier . '</a>
        <div class="dropdown-divider"></div>
        <a class="dropdown-item delete" id="'.$row["id_type_plat"].'" href="#" data-status="'.$row["statut_type_plat"].'">'. $changer_statut . '</a>
      </div>
    </div>
...
{
	$statement = $connect->prepare("SELECT * FROM type_plat"); // same query as above
	$statement->execute();
